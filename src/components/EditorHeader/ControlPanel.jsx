...
export default function ControlPanel({
                ...
                if (a.component === "field") {
                    ...
                    updateTable(a.tid, {
                        fields: tables[a.tid].fields
                            .filter(
                                (e) => e.id !== tables[a.tid].fields.length - 1,
                            )
                            .map((t, i) => ({ ...t, id: i })),
                    });
                    ...
                    updateTable(a.tid, {
                        indices: tables[a.tid].indices
                            .filter(
                                (e) =>
                                    e.id !== tables[a.tid].indices.length - 1,
                            )
                            .map((t, i) => ({ ...t, id: i })),
                    });
    ...
    const copy = () => {
        switch (selectedElement.element) {
            case ObjectType.TABLE:
                navigator.clipboard
                    .writeText(
                        JSON.stringify({ ...tables[selectedElement.id] }),
                    )
                    .catch(() => {
        ...
        File: {
            ...
            "Export as": {
                children: [
                    {
                        PNG: () => {
                            toPng(document.getElementById("canvas")).then(
                                function (dataUrl) {
                                    setExportData((prev) => ({
                                        ...prev,
                                        data: dataUrl,
        ...
        Settings: {
            ...
            Panning: {
                ...
                function: () =>
                    setSettings((prev) => {
                        Toast.success(
                            `Panning is ${settings.panning ? "off" : "on"}`,
                        );
    ...
    useHotkeys("ctrl+shift+g, meta+shift+g", viewGrid, {
        preventDefault: true,
    });
                            ...
                            onClick={() => {
                                ...
                                if (body.hasAttribute("theme-mode")) {
                                    if (
                                        ...
                                        menu["View"]["Theme"].children[1][
                                            "Dark"
                                        ]();
                                    } else {
                                        menu["View"]["Theme"].children[0][
                                            "Light"
                                        ]();
                                    }
                                }
