        ...
        if (a.action === Action.ADD) {
            ...
            if (a.element === ObjectType.AREA) {
                ...
                if (a.component === "field") {
                    ...
                    updateTable(a.tid, {
                        indices: updatedIndices.map((t, i) => ({
                            ...t,
                            id: i,
                        })),
                    });
                    ...
                    if (a.updatedFields) {
                        if (a.undo.name) {
                            a.updatedFields.forEach((x) =>
                                updateField(x.tid, x.fid, {
                                    type: a.undo.name.toUpperCase(),
                                }),
                            );
                        }
                    }
    ...
    const fitWindow = () => {
        ...
        tables.forEach((table) => {
            ...
            minMaxXY.maxX = Math.max(
                minMaxXY.maxX,
                table.x + settings.tableWidth,
            );
            minMaxXY.maxY = Math.max(
                minMaxXY.maxY,
                table.y + getTableHeight(table),
            );
        });
                        ...
                        <div className="flex items-center ms-3 gap-2">
                            {databases[database].image && (
                                <img
                                    ...
                                    title={
                                        databases[database].name + " diagram"
                                    }
                                />
                            )}
                            <div
                                ...
                                onPointerLeave={(e) =>
                                    e.isPrimary && setShowEditName(false)
                                }
                                onPointerDown={(e) => {
