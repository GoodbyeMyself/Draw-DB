...
export default function Canvas() {
    ...
    const handlePointerDownOnElement = (e, id, type) => {
        ...
        if (
            bulkSelectedElements.some((el) => el.id === id && el.type === type)
        ) {
        ...
        if (panning.isPanning) {
            setTransform((prev) => ({
                ...
                pan: {
                    x:
                        panning.panStart.x +
                        (panning.cursorStart.x - pointer.spaces.screen.x) /
                            transform.zoom,
                    y:
                        panning.panStart.y +
                        (panning.cursorStart.y - pointer.spaces.screen.y) /
                            transform.zoom,
                },
            }));
            return;
        ...
        if (areaResize.id !== -1) {
            ...
            switch (areaResize.dir) {
                ...
                case "tl":
                    ...
                    newDims.y = y;
                    newDims.width =
                        areaInitDimensions.width - (x - areaInitDimensions.x);
                    newDims.height =
    ...
    useEventListener(
        ...
        (e) => {
            ...
            // 获取当前鼠标在画布中的位置
            const mouseX = pointer.spaces.diagram.x;
            const mouseY = pointer.spaces.diagram.y;

            // 计算新的缩放级别
            const newZoom = Math.max(
                0.1,
                Math.min(5.0, transform.zoom * zoomFactor),
            );

            // 计算缩放中心点（鼠标位置）
            const scaleCenterX = mouseX;
            const scaleCenterY = mouseY;

            // 计算新的平移位置，使缩放以鼠标位置为中心
            const newPanX =
                scaleCenterX -
                (scaleCenterX - transform.pan.x) * (newZoom / transform.zoom);
            const newPanY =
                scaleCenterY -
                (scaleCenterY - transform.pan.y) * (newZoom / transform.zoom);
        ...
        <div className="grow h-full touch-none" id="canvas">
            <div
                ...
                style={{
                    ...
                    backgroundColor:
                        settings.mode === "dark" ? darkBgTheme : "white",
                }}
            >
                <svg
                    id="diagram"
